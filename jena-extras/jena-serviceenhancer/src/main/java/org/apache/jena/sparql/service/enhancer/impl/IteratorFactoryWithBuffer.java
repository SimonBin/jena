...
public class IteratorFactoryWithBuffer<T, I extends Iterator<T>>
{
    ...
    public static <T, I extends Iterator<T>> SubIterator<T, I> wrap(I delegate) {
        return new IteratorFactoryWithBuffer<T, I>(delegate).createSubIterator(false);
    }
...
//            }
    ...
    protected class SubIteratorImpl
        ...
        implements PeekingIterator<T>, SubIterator<T, I>
    {
    ...
    protected void checkShrink() {
        // TODO Shrink e.g. if the needed size of the buffer has halved
//        SortedSet<Long> keys = (SortedSet<Long>)offsetToChild.asMap().keySet();
//        if (keys.first()) {
//
//        }
    ...
    public static void main(String[] args) {
        ...
        SubIterator<Integer, ?> primary = factory.createSubIterator(false);

        for (int i = 0; i < 2; ++i) {
            System.out.println("primary: " + primary.next());
        }

        SubIterator<Integer, ?> secondary = primary.createSubIterator(false);
        for (int i = 0; i < 2; ++i) {
            System.out.println("secondary: " + secondary.next());
        }

        // secondary.close();
        SubIterator<Integer, ?> ternary = secondary.createSubIterator(false);
