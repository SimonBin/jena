 ...
 */
public abstract class QueryIterRepeatApplyBulk extends QueryIter1 {
    protected int count = 0;
    protected QueryIterator currentStage;
    protected volatile boolean cancelRequested = false;

    public QueryIterRepeatApplyBulk(QueryIterator input, ExecutionContext context) {
        super(input, context);
    ...
    protected abstract QueryIterator nextStage(QueryIterator input);
    ...
    private QueryIterator makeNextStage() {
        ...
        QueryIterator it = getInput();
        QueryIterator iter = nextStage(it);
        return iter;
    }
