...
public interface IndexNode<ComponentType> {
    ...
    // Some build a recursion such that at the end we create a new mutable tuple result object (an array) and then
    // ask all the parents to add their contributions to it
    // The root yields a contribution that corresponds to a single tuple with zero elements
