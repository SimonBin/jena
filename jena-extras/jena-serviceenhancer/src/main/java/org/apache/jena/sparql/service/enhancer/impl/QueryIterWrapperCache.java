...
//        this.isLoopJoin = isLoopJoin;
        this.inputBatch = inputBatch;
        this.idxVar = idxVar;
        this.idxVarSet = Collections.singleton(idxVar);
    ...
    protected void setupForNextLhsBinding() {
        ...
        if (mergeLeftJoin.hasNext()) {
            ...
            if (!BatchQueryRewriter.isRemoteEndMarker(outputId)) {
                ...
                Binding inputBinding = inputPart.getPartition();
                // System.out.println("Moving to inputBinding " + inputBinding);
//                Binding joinBinding = new BindingProject(joinVars, inputBinding);
//
//                ServiceCacheKey cacheKey = new ServiceCacheKey(serviceNode, op, joinBinding, isLoopJoin);
    ...
    public void prepareNextBatch() {
        ...
        // Get the known result set size
        // Estimate<Long> currentLimit = cache.getLimit(serviceNode);
        ...
        while (mergeLeftJoin.hasNext() && remainingBatchCapacity > 0) {
            ...
            if (rhs != null) {
                while (rhs.hasNext() && arrLen < remainingBatchCapacity) {
                    ...
                    Binding outputBinding = BindingUtils.project(rawOutputBinding, rawOutputBinding.vars(), idxVarSet);
            ...
            // Update the known result set size: If we exhausted the result set without an end marker we know the exact result size limit
            // Otherwise, may we increase the known result set size limit
//            if (isRhsExhausted) {
//                System.err.println("rhsExhausted");
//            }
            /*
            // result set size detection is performed on BatchRequestIterator -
            // we don't have to deal with it here
            if (isRhsExhausted) {
                cache.updateLimit(serviceNode, new Estimate<>(processedBindingCount, true));
            } else {
                if (processedBindingCount > currentLimit.getValue()) {
                    cache.updateLimit(serviceNode, new Estimate<>(processedBindingCount, false));
                }
            }
            */
...
//                for (Object item : arr) {
//                    System.out.println("Cache write: " + item);
//                }
                ...
                if (isRhsExhausted) {
                    ...
                    // TODO a key is also completed if in total fewer tuples than
